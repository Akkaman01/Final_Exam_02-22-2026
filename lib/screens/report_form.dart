...
class _ReportFormScreenState extends State<ReportFormScreen> {
  ...
  Future<void> _init() async {
    ...
    if (widget.editReportId != null) {
      final row = await SqliteService.instance.getReportByIdJoin(widget.editReportId!);
      if (row != null) {
  ...
  Future<void> _runAi() async {
    ...
    final mappedTypeName = AiInferenceService.instance.mapResultToViolationName(result);
    String? mappedTypeId;
    if (mappedTypeName.isNotEmpty) {
      mappedTypeId = await SqliteService.instance.findViolationTypeIdByName(mappedTypeName);
    }
  ...
  Future<void> _save() async {
    ...
    final report = IncidentReport(
      ...
      evidencePhoto: _photoCtl.text.trim().isEmpty ? null : _photoCtl.text.trim(),
      timestamp: _nowText(),
      aiResult: _aiResultCtl.text.trim().isEmpty ? null : _aiResultCtl.text.trim(),
      aiConfidence: aiConf,
    );
    ...
    final onlineResult = await FirebaseService.instance.saveIncidentOnline(report.toMap());

    if (!mounted) return;
    setState(() => _saving = false);
    if (onlineResult.success) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Saved: Offline + Online')),
      );
                ...
                child: ListView(
                  children: [
                    DropdownButtonFormField<String>(
                      ...
                      validator: (v) =>
                          (v == null || v.isEmpty) ? 'Please select a polling station' : null,
                    ),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      validator: (v) =>
                          (v == null || v.isEmpty) ? 'Please select a violation type' : null,
                    ),
                    const SizedBox(height: 12),
                    FormTextField(
                      label: 'Reporter Name',
                      ...
                      validator: (v) =>
                          (v == null || v.trim().isEmpty) ? 'Please enter reporter name' : null,
                    ),
                    const SizedBox(height: 12),
                    FormTextField(
                      label: 'Description',
