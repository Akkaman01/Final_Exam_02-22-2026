...
class FirebaseService {
  ...
  Future<OnlineSaveResult> saveIncidentOnline(Map<String, Object?> payload) async {
    try {
      ...
      debugPrint('Unknown online save error: $e');
      return OnlineSaveResult(
        success: false,
  ...
  Future<OnlineSaveResult> deleteIncidentOnlineByReporterName(
    ...
    try {
      await _ensureInitialized();
      final collection = FirebaseFirestore.instance.collection('incident_report');
      final docId = _safeDocIdFromReporter(reporterName);

      // Remove the deterministic doc id first.
      await collection.doc(docId).delete();

      // Remove any legacy docs created with random ids but same reporter_name.
      ...
      debugPrint('Firebase delete failed: ${e.code} ${e.message}');
      return OnlineSaveResult(
        success: false,
        message: 'Firebase error: ${e.code} ${e.message ?? ''}'.trim(),
      );
    } catch (e) {
      debugPrint('Unknown online delete error: $e');
      return OnlineSaveResult(
        success: false,
        message: 'Unknown error: $e',
      );
